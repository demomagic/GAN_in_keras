...
class GAN():
    ...
    def build_training_method(self):
        real_image = tf.placeholder(tf.float32, shape=[None, self.image_width, self.image_height, self.channel], name='real_image')
        random_input = tf.placeholder(tf.float32, shape=[None, self.latent_dim], name='rand_input')
        ...
        for epoch in range(epochs):
            for _ in range(batch_num):
                ...
                for _ in range(d_iters):
                    ...
                    _, d_loss = self.sess.run([self.d_opt, self.d_loss], feed_dict={self.real_image: images, self.random_input: noise})
                              
                self.dis.trainable = False
                _, g_loss = self.sess.run([self.g_opt, self.g_loss], feed_dict = {self.random_input: noise})
